...
def ddisasm_cmd(conf):
    ...
    cmd2 = 'sudo docker run --rm -v %s/:/input  -v %s/:/output grammatech/ddisasm:1.5.2 sh -c "ddisasm /input/%s --asm /output/%s"'%(conf.bench, conf.tool_path, bfile, rfile)
    cmd_list.append(cmd2)
    return cmd_list


class Ddisasm(Manager):
    def __init__(self, core, bench='/data3/1_reassessor/benchmark', tool_path='/data3/1_reassessor/dataset/ddisasm', tool_name='ddisasm'):
