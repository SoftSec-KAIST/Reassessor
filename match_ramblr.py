...
from utils import *
import pickle
from parser import parse_att_components
...
def address_src_file(src_path, is_pie, relocs, elf):
    lines = trim_lines(src_path, is_pie)
    addressed_lines = address_labels(lines, relocs)
    addressed_asms = []
    addressed_data = []
...
def ____get_label_expr(addressed_labels, s):
    s = reduce_expr(s)
    digits = "-0123456789"
    if '[' in s:
        s = s.split('[')[1].split(']')[0]
...
def has_label(s, relocs):
    if s:
         return (s in relocs) or s.startswith('.label') or s.startswith('label') or s.startswith('sub')
    return False
...
def parse_source(prog, elf, cs, src_path):
    ...
    addressed_asms, addressed_data = address_src_file(src_path, is_pie, relocs, elf)
    print('address_src_file done')

    text_end = prog.text_base + len(prog.text_data)
    for i in range(len(addressed_asms)):
        ...
        try:
            ...
            #components = parse_intel_components(relocs, inst, tokens)
            ...
            #import pdb
            #pdb.set_trace()
            #labels =  ','.join(item.get_labels()[0].get_name() for item in components if item.is_ms())
            #if labels:
            #    print('%s %s'%(hex(addr), labels))
        except:
            print('error %s %s'%(hex(addr), line))
            pass

    print('address_asms done')
    for addr, exprs, size, line in addressed_data:
        terms = parse_labels(relocs, 0, exprs)
